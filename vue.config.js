...
module.exports = {
	...
	css: {
		loaderOptions: {
			sass: {
				// @/ 是 src/ 的别名
				data: fs.readFileSync(path.resolve(__dirname, `./public/assets/styles/variables.scss`), 'utf-8') // 公共变量文件注入
			}
		}
	},
	...
	chainWebpack: config => {
		...
		config.resolve.alias
			...
			.set('@editor', path.resolve('editor'))
			.set('@assets', path.resolve('./public/assets'))
			.set('@components-lib', path.resolve('./components-lib'))
			.set('@editor-components-lib', path.resolve('./editor-components-lib'))
			config.module
			.rule('js')
