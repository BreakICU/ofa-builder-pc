...
const actions = {
    ...
    setProjectData({commit, state, dispatch}, data){
        ...
        if(!projectData){
            projectData = Project.getProjectConfig();
        }
    ...
    copyPage({commit}, uuid){
		...
		let data = Page.copyPage(pageData)
	...
	addElement({commit}, elData){
		...
		let data = Element.createElement(elData, {zIndex: activePage.elements.length + 1})
    ...
    copyElement({state, commit}, elData){
		...
		let activePage = getters.activePage(state)
		let data = Element.copyElement(copyOrignData, {zIndex: activePage.elements.length + 1})
		commit('addElement', data);
		commit('setActiveElementUUID', data.uuid)
		commit('addHistoryCache')
	},
